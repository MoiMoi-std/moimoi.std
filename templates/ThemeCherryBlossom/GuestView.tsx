...
export default function CherryBlossomGuestView({ wedding, guestName = '', rsvpId }: TemplateProps) {
  ...
  if (!wedding) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff5f9' }}>
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ width: 48, height: 48, marginBottom: 12, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><div style={{ width: 24, height: 32, borderRadius: '50% 0 50% 0', background: '#d4507a', opacity: 0.6, transform: 'rotate(-15deg)' }} /></div>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', overflow: 'hidden', height: 380 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(90,20,45,0.22) 0%, rgba(30,5,18,0.78) 100%)', zIndex: 1 }} />
          {/* CSS petal shapes in corners */}
          {[{ top: 12, left: 12 }, { top: 12, right: 12 }, { bottom: 12, left: 12 }, { bottom: 12, right: 12 }].map((pos, i) => (
            <div key={i} style={{
              position: 'absolute', zIndex: 2, width: 22, height: 28, opacity: 0.28,
              background: '#fff', clipPath: 'ellipse(40% 50% at 50% 50%)',
              transform: `rotate(${i * 90}deg)`, ...pos
            }} />
          ))}
          {/* Text content */}
          <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 24px', textAlign: 'center' }}>
            <p style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.65)', letterSpacing: '0.38em', textTransform: 'uppercase', marginBottom: 20 }}>
              TRÂN TRỌNG KÍNH MỜI
            </p>
            <h1 style={{
        ...
        <div style={{ maxWidth: 540, margin: '0 auto', padding: '0 16px 72px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className='c-fade c-card' style={{ padding: '26px 22px' }}>
            <p style={{ fontSize: 10, fontWeight: 700, color: pink, letterSpacing: '0.2em', textTransform: 'uppercase', textAlign: 'center', marginBottom: 18 }}>
              Thông tin sự kiện
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {([
                ...
                <div key={label} style={{
                  ...
                  <div>
                    <span style={{ fontSize: 9, fontWeight: 700, color: pink, textTransform: 'uppercase', letterSpacing: '0.14em', display: 'block', marginBottom: 3 }}>
                      {label}
                    </span>
          ...
          {!submitted ? (
            <div className='c-fade c-card' style={{ padding: '28px 22px' }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: pink, letterSpacing: '0.2em', textTransform: 'uppercase', textAlign: 'center', marginBottom: 4 }}>
                Xác nhận tham dự
              </p>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                ...
                <div>
                  <label style={labelStyle}>
                    Số điện thoại{' '}
                    <span style={{ color: '#c9a0b4', fontSize: 11, fontWeight: 500, textTransform: 'none', letterSpacing: 0 }}>(tùy chọn)</span>
                  </label>
                    ...
                    {[1, 2, 3, 4, 5].map((n, i) => (
                      <button
                        ...
                        style={{
                          ...
                          background: partySize === n ? `linear-gradient(135deg, ${pinkDark}, ${pink})` : 'rgba(255,255,255,0.7)',
                ...
                {submitError && (
                  <div style={{ padding: '12px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 14, color: '#dc2626', fontSize: 14, textAlign: 'center' }}>
                    {submitError}
                  </div>
                )}
              </form>
            ...
            <div className='c-fade c-card' style={{ padding: '52px 28px', textAlign: 'center' }}>
              <svg style={{ color: pinkDark, marginBottom: 20, opacity: 0.7 }} width="44" height="44" viewBox="0 0 48 48" fill="currentColor">
              ...
              {wish && (
                <div style={{ marginTop: 28, padding: '14px 20px', background: `${pink}0d`, borderRadius: 16, borderLeft: `3px solid ${pink}`, textAlign: 'left' }}>
                  <p style={{ fontSize: 9, fontWeight: 700, color: pink, textTransform: 'uppercase', letterSpacing: '0.14em', marginBottom: 8 }}>
                    Lời chúc của bạn
                  </p>
