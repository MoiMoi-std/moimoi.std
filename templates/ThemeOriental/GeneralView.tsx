...
export default function OrientalGeneralView({ wedding }: TemplateProps) {
  ...
  return (
    <>
      ...
      <div style={{ background: ricePaper, fontFamily: "'Be Vietnam Pro', sans-serif", color: textDark, overflowX: 'hidden' }}>
        ...
        <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', textAlign: 'center' }}>
          {mergedContent.cover_image && (
            <div className="or-zoom" style={{ position: 'absolute', inset: 0, backgroundImage: `url(${mergedContent.cover_image})`, backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.35) saturate(0.7) sepia(0.3)' }} />
          )}
          {!mergedContent.cover_image && (
            <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(160deg, ${darkRed} 0%, ${red} 40%, #3a0a0a 100%)` }} />
          )}
          <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(to bottom, rgba(92,10,10,0.55) 0%, rgba(92,10,10,0.15) 40%, rgba(92,10,10,0.7) 75%, rgba(92,10,10,0.95) 100%)` }} />

          {/* Cloud border top */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 6, background: `linear-gradient(to right, ${gold}, ${goldLight}, ${gold})`, opacity: 0.7 }} />
          ...
          <div className="or-float" style={{ position: 'absolute', top: '12%', right: '10%', zIndex: 1, opacity: 0.45, animationDelay: '1.5s' }}>
          ...
          <div style={{ position: 'relative', zIndex: 2, padding: '60px 28px' }}>
            ...
            <div className="or-up or-d1" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <p className="or-up or-d1" style={{ fontSize: 9, letterSpacing: '0.55em', color: gold, textTransform: 'uppercase', marginBottom: 20, fontFamily: "'Noto Serif SC', serif" }}>Trân trọng kính mời</p>
            <h1 className="or-up or-glow or-d2" style={{ fontSize: 'clamp(2.8rem,11vw,6.5rem)', fontFamily: "'Noto Serif SC', serif", fontWeight: 400, color: goldLight, lineHeight: 1, marginBottom: 10, letterSpacing: '0.03em' }}>
              {mergedContent.groom_name || 'Chú Rể'}
            </h1>
            <div className="or-up or-d2" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, margin: '16px 0' }}>
              <div style={{ width: 32, height: 1, background: `rgba(212,168,48,0.5)` }} />
              <svg width="20" height="20" viewBox="0 0 20 20"><path d="M10 2 C7 2 4 5 4 8 C4 13 10 18 10 18 C10 18 16 13 16 8 C16 5 13 2 10 2 Z" fill="none" stroke={gold} strokeWidth="0.8" /><circle cx="10" cy="8" r="2" fill={gold} opacity="0.7" /></svg>
              <div style={{ width: 32, height: 1, background: `rgba(212,168,48,0.5)` }} />
            </div>
            <h1 className="or-up or-glow or-d3" style={{ fontSize: 'clamp(2.8rem,11vw,6.5rem)', fontFamily: "'Noto Serif SC', serif", fontWeight: 400, color: goldLight, lineHeight: 1, marginBottom: 28, letterSpacing: '0.03em' }}>
              {mergedContent.bride_name || 'Cô Dâu'}
            </h1>
            <div className="or-up or-d3" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 20 }}>
              <div style={{ width: 36, height: 1, background: `rgba(212,168,48,0.5)` }} />
              <div style={{ width: 4, height: 4, background: gold, transform: 'rotate(45deg)', opacity: 0.8 }} />
              <div style={{ width: 36, height: 1, background: `rgba(212,168,48,0.5)` }} />
            </div>
            {mergedContent.wedding_date && (
              <p className="or-up or-d4" style={{ fontSize: 13, letterSpacing: '0.12em', color: `rgba(240,204,96,0.75)`, fontFamily: "'Noto Serif SC', serif" }}>
        ...
        {timeRemaining && (
          <section style={{ background: darkRed, padding: '60px 24px', textAlign: 'center' }}>
            <p style={{ fontSize: 9, letterSpacing: '0.5em', color: gold, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 28 }}>Đếm ngược ngày hỷ sự</p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, maxWidth: 360, margin: '0 auto' }}>
              {[{ v: timeRemaining.days, l: 'Ngày' }, { v: timeRemaining.hours, l: 'Giờ' }, { v: timeRemaining.minutes, l: 'Phút' }, { v: timeRemaining.seconds, l: 'Giây' }].map((it, i) => (
                <div key={i} style={{ flex: 1 }}>
                  <div style={{ background: 'rgba(212,168,48,0.08)', border: `1px solid rgba(212,168,48,0.35)`, borderRadius: 4, padding: '16px 4px', marginBottom: 8 }}>
                    <span style={{ display: 'block', fontSize: 'clamp(1.8rem,6vw,3rem)', fontFamily: "'Noto Serif SC', serif", color: goldLight, lineHeight: 1 }}>{String(it.v).padStart(2, '0')}</span>
                  </div>
                  <p style={{ fontSize: 9, letterSpacing: '0.25em', color: 'rgba(212,168,48,0.65)', textTransform: 'uppercase' }}>{it.l}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section style={{ background: ricePaper, padding: '72px 24px' }}>
          <div style={{ maxWidth: 480, margin: '0 auto', textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: 'clamp(1.8rem,6vw,3.2rem)', fontFamily: "'Noto Serif SC', serif", color: red, marginBottom: 20 }}>
              {mergedContent.groom_name} & {mergedContent.bride_name}
            </h2>
            <p style={{ fontSize: 15, lineHeight: 1.9, color: textMid }}>
              {mergedContent.intro_text || 'Với tất cả tình yêu và sự trân trọng từ hai gia đình, chúng tôi kính mời quý vị đến chung vui ngày hỷ sự của chúng tôi. Hạnh phúc sẽ được nhân đôi khi có sự hiện diện của quý vị.'}
            </p>
          </div>
        </section>
        ...
        {(mergedContent.groom_father || mergedContent.bride_father) && (
          <section style={{ background: riceDark, padding: '56px 24px' }}>
            <div style={{ maxWidth: 480, margin: '0 auto' }}>
              <p style={{ fontSize: 9, letterSpacing: '0.45em', color: gold, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 28, textAlign: 'center' }}>Gia đình thông gia</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                {[
                  ...
                  <div key={i} style={{ border: `1px solid rgba(212,168,48,0.3)`, borderRadius: 4, padding: 16, textAlign: 'center', background: ricePaper }}>
                    <p style={{ fontSize: 10, letterSpacing: '0.25em', color: red, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 10 }}>{fam.side}</p>
        ...
        <section style={{ background: darkRed, padding: '72px 24px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -40, right: -40, width: 150, height: 150, borderRadius: '50%', border: `1px solid rgba(212,168,48,0.15)` }} />
          <div style={{ position: 'absolute', bottom: -30, left: -30, width: 120, height: 120, borderRadius: '50%', border: `1px solid rgba(212,168,48,0.1)` }} />
          <div style={{ maxWidth: 480, margin: '0 auto', position: 'relative', zIndex: 1 }}>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <p style={{ fontSize: 9, letterSpacing: '0.5em', color: gold, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 10 }}>Kính mời quý vị</p>
              <h2 style={{ fontSize: 'clamp(1.6rem,5vw,2.6rem)', fontFamily: "'Noto Serif SC', serif", color: goldLight }}>Lễ thành hôn</h2>
            </div>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 14, padding: '14px 0', borderBottom: `1px solid rgba(212,168,48,0.12)` }}>
            ...
            {mergedContent.map_url && (
              <a href={mergedContent.map_url} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 24, padding: '12px 28px', border: `1px solid ${gold}`, color: gold, textDecoration: 'none', fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif" }}>
                Xem bản đồ
              </a>
            )}
          </div>
          ...
          <section style={{ background: ricePaper, padding: '72px 24px' }}>
            <div style={{ maxWidth: 480, margin: '0 auto' }}>
              <div style={{ textAlign: 'center', marginBottom: 32 }}>
                <p style={{ fontSize: 9, letterSpacing: '0.45em', color: gold, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 8 }}>Album ảnh</p>
                <h2 style={{ fontSize: 'clamp(1.6rem,5vw,2.6rem)', fontFamily: "'Noto Serif SC', serif", color: red }}>Kỷ niệm đáng nhớ</h2>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                {albumImages.slice(0, 4).map((img: string, i: number) => (
                  <div key={i} style={{ aspectRatio: '3/4', overflow: 'hidden', border: `2px solid rgba(212,168,48,0.4)`, boxShadow: '0 4px 16px rgba(92,10,10,0.15)' }}>
                    <img src={img} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'sepia(0.15) saturate(1.05)' }} />
                  </div>
                ))}
              </div>
            </div>
        ...
        {(mergedContent.bank_name || mergedContent.account_number) && (
          <section style={{ background: riceDark, padding: '72px 24px' }}>
            <div style={{ maxWidth: 420, margin: '0 auto', textAlign: 'center' }}>
              <p style={{ fontSize: 9, letterSpacing: '0.45em', color: gold, textTransform: 'uppercase', fontFamily: "'Noto Serif SC', serif", marginBottom: 14 }}>Hộp phong bì mừng cưới</p>
              <h2 style={{ fontSize: 'clamp(1.6rem,5vw,2.4rem)', fontFamily: "'Noto Serif SC', serif", color: red, marginBottom: 8 }}>Tấm lòng quý khách</h2>
              <p style={{ fontSize: 13, color: textMid, marginBottom: 28, fontStyle: 'italic' }}>Sự hiện diện của quý vị là niềm vui lớn nhất của đôi uyên ương.</p>
              <div style={{ border: `1px solid rgba(212,168,48,0.4)`, borderRadius: 4, padding: '24px 20px', background: ricePaper, boxShadow: `inset 0 0 0 3px rgba(212,168,48,0.08)` }}>
                {mergedContent.account_name && <p style={{ fontSize: 17, fontFamily: "'Noto Serif SC', serif", color: red, marginBottom: 6 }}>{mergedContent.account_name}</p>}
                ...
                {mergedContent.account_number && <p style={{ fontSize: 22, color: textDark, letterSpacing: '0.08em', fontFamily: "'Noto Serif SC', serif" }}>{mergedContent.account_number}</p>}
              </div>
            </div>
          </section>
        )}
        ...
        {/* ── FOOTER ── */}
        <section style={{ background: darkRed, borderTop: `2px solid rgba(212,168,48,0.3)`, padding: '52px 24px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 'clamp(1.2rem,4vw,2rem)', fontFamily: "'Noto Serif SC', serif", color: goldLight, marginBottom: 6 }}>
            {mergedContent.groom_name} & {mergedContent.bride_name}
          </p>
          {mergedContent.wedding_date && (
            <p style={{ fontSize: 10, letterSpacing: '0.3em', color: `rgba(212,168,48,0.5)`, textTransform: 'uppercase' }}>
